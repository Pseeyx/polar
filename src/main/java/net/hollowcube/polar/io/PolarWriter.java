...
public class PolarWriter {
            ...
            content -> {
              ...
              for (var chunk : world.chunks()) {
                writeChunk(
                    content,
                    chunk,
                    world.maxSection() - world.minSection() + 1,
  ...
  private static void writeChunk(
      @NotNull NetworkBuffer buffer, @NotNull PolarChunk chunk, int sectionCount, short version) {
    VarIntUtil.writeChunkCoord(buffer, chunk.x(), version);
    VarIntUtil.writeChunkCoord(buffer, chunk.z(), version);
...

    {
      ...
      for (int i = 0; i < PolarChunk.MAX_HEIGHTMAPS; i++) {
        if (chunk.heightmap(i) != null) heightmapBits |= 1 << i;
      }
  ...
  private static void writeBlockEntity(
      @NotNull NetworkBuffer buffer, @NotNull PolarChunk.BlockEntity blockEntity) {
