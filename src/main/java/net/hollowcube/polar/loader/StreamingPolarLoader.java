...
import net.hollowcube.polar.io.VarIntUtil;
...
final class StreamingPolarLoader {
  static final NetworkBuffer.Type<String[]> STRING_ARRAY =
  ...
  int plainsBiomeId;
  @NonFinal int version, dataVersion;
  ...
  public void loadAllSequential(@NotNull ReadableByteChannel channel, long fileSize)
      throws IOException {
    try (Arena dstArena = Arena.ofConfined()) {
          ...
          while (offset < fileSize) {
            ...
            if (n < 0) {
              throw new EOFException(
                  "Unexpected EOF: expected " + fileSize + " bytes, got " + offset);
            }
            offset += n;
          }
      ...
      // Now we can just read the dst buffer without having to worry about the extra footprint of
      // src
      readData(dst);
    }
  }
  ...
  private void readData(@NotNull MemorySegment segment) {
    ...
    if (version > PolarWorld.VERSION_WORLD_USERDATA) {
      ...
      if (worldAccess != null) {
        var worldDataView =
            NetworkBuffer.wrap(
                segment.asSlice(buffer.readIndex(), userDataLength), 0L, userDataLength);
  ...
  private void readChunk(
      ...
      int minSection,
      int maxSection) {
    final var chunkX = VarIntUtil.readChunkCoord(buffer, (short) version);
    final var chunkZ = VarIntUtil.readChunkCoord(buffer, (short) version);
    ...
    try {
      ...
      for (int i = 0; i < blockEntityCount; i++) {
        ...
        if (chunkEntries != null && chunkTickables != null) {
          ...
          final int index =
              CoordConversion.chunkBlockIndex(blockEntity.x(), blockEntity.y(), blockEntity.z());
  ...
  private void readSection(
      ...
      for (int y = 0; y < CHUNK_SECTION_SIZE; y++) {
        for (int z = 0; z < CHUNK_SECTION_SIZE; z++) {
          for (int x = 0; x < CHUNK_SECTION_SIZE; x++) {
            ...
            // Vanilla block entities must be tracked in the chunk entries so they are sent to the
            // client.
    ...
    if (version > PolarWorld.VERSION_UNIFIED_LIGHT) {
      var blockLightContent =
          version >= PolarWorld.VERSION_IMPROVED_LIGHT
              ? PolarSection.LightContent.VALUES[buffer.read(BYTE)]
              : (buffer.read(BOOLEAN)
                  ? PolarSection.LightContent.PRESENT
                  : PolarSection.LightContent.MISSING);
      byte[] blockLight =
          blockLightContent == PolarSection.LightContent.PRESENT ? buffer.read(LIGHT_DATA) : null;
      if (loadLighting && blockLightContent != PolarSection.LightContent.MISSING)
        unsafeUpdateBlockLightArray(
            section.blockLight(), getLightArray(blockLightContent, blockLight));
      ...
      var skyLightContent =
          version >= PolarWorld.VERSION_IMPROVED_LIGHT
              ...
              : (buffer.read(BOOLEAN)
                  ...
                  : PolarSection.LightContent.MISSING);
      byte[] skyLight =
          skyLightContent == PolarSection.LightContent.PRESENT ? buffer.read(LIGHT_DATA) : null;
  ...
  private int[] readBlockPalette(@NotNull NetworkBuffer buffer) {
    ...
    for (int i = 0; i < rawBlockPalette.length; i++) {
      blockPalette[i] =
          blockToStateIdCache.computeIfAbsent(
              rawBlockPalette[i],
              (String key) -> {
                try {
  ...
  private int[] readBiomePalette(@NotNull NetworkBuffer buffer) {
    ...
    for (int i = 0; i < rawBiomePalette.length; i++) {
      biomePalette[i] =
          biomeToIdCache.computeIfAbsent(
              rawBiomePalette[i],
              (String name) -> {
                PolarWorldAccess searchWorldAccess =
                    Objects.requireNonNullElse(this.worldAccess, PolarWorldAccess.DEFAULT);
