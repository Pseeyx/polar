...
public interface PolarDataConverter {
  @NotNull PolarDataConverter NOOP = new PolarDataConverter() {};

  /**
   * Returns the data version to use on worlds lower than {@link PolarWorld#VERSION_DATA_CONVERTER}
   * which do not store a data version. Defaults to the current Minestom data version.
   */
  ...
  default @NotNull Map.Entry<String, CompoundBinaryTag> convertBlockEntityData(
      @NotNull String id, @NotNull CompoundBinaryTag data, int fromVersion, int toVersion) {
